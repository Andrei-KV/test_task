   ...
   # В реальной системе 'ID_...' — это ID документа из PostgreSQL.
    PROMPT_MAPPING = {
        "ID_DEFAULT": (
            ...
            "5.  **Атрибуция (Источники):** В самом конце ответа, после горизонтальной черты (`---`), создай секцию `### Использованные Источники`.\n"
            "    * **ОБЯЗАТЕЛЬНО:** Укажи **номера страниц** (из переменной `{pages_str}`) и **названия разделов** (из переменной `{sections_str}`), если они доступны в контексте. Это критически важно.\n"
            "    * После этого перечисли конкретные **части документа** (например, `Пункт 1.1`, `Глава 2`), на которые ты ссылался.\n"
            "    * В конце секции добавь **веб-ссылку** на полный документ, используя `[Название документа]({web_link})`.\n\n"
            "6. **Сокращение Вводных Фраз:** Исключи фразы типа 'Отличный вопрос. Это фундаментальное понятие...'. Система-промпт 'Эксперт', подразумевает, что модель сразу переходит к фактам.\n"
