...
class PromptManager:
   ...
   # В реальной системе 'ID_...' — это ID документа из PostgreSQL.
    ...
    1. Если ты **не можешь** дать полный, подтвержденный фактами ответ, используя исключительно `<КОНТЕКСТ>`, ты должен начать ответ со **СТРОГОЙ** фразы: **[CLARIFICATION_NEEDED]**
...
Если ответ в тексте не найден, так и напиши: «[CLARIFICATION_NEEDED]».
...
**ОТВЕТ может быть неточным. [CLARIFICATION_NEEDED] [стр.X]**
...
class RAGService:
    ...
    async def aquery(self, user_query: str, low_precision: bool = False) -> tuple[str, str | None, float, str, list[int] | None]:
        ...
        # 6. Post-processing: Check if the answer indicates missing information
        # If the LLM says it can't find the answer, we should downgrade the score
        # to trigger the clarification logic in the router.
        
        if "[CLARIFICATION_NEEDED]" in final_answer:
            logger.info("LLM indicated that the answer is missing or incomplete ([CLARIFICATION_NEEDED]). Downgrading score.")
            score = 0.5  # Force low score to trigger clarification
            # Optional: Remove the tag from the answer if you don't want to show it to the user
            # final_answer = final_answer.replace("[CLARIFICATION_NEEDED]", "").strip()
