        ...
        while True:
            ...
            if score < 0.7:
                ...
                if clarification_count < 2:
                    ...
                    # Важно сбросить контекст перед тем, как давать ответ низкой точности
                    ...
                    await manager.send_personal_message(text=final_answer, websocket=websocket, web_link=web_link, title=title, page_numbers=page_numbers, sections=sections)
                    await context_manager.reset_context(client_id)
            else:
                await context_manager.add_message(client_id, "bot", answer)
                await manager.send_personal_message(text=answer, websocket=websocket, web_link=web_link, title=title, page_numbers=page_numbers, sections=sections)
                await context_manager.reset_context(client_id)
